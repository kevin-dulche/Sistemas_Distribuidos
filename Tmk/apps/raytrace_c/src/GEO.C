...
/*
 * $Id: geo.c,v 1.4 1997/12/21 19:14:16 alc Exp $
 */
...
CHAR	*GeoFileName;
	...
	while ((stat = fscanf(pf, "%s %s", objstr, objname)) != EOF)
		...
		curr	    = GlobalMallocSec(sizeof(OBJECT), "geo.c");
		...
		ps = GlobalMallocSec(sizeof(SURF), "geo.c");
		curr->surf = ps;
		...
		pe = GlobalMallocSec(sizeof(ELEMENT)*curr->numelements, "geo.c");
		curr->pelem = pe;
