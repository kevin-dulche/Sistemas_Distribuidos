...
/*
 * $Id: memory.c,v 1.2 1997/12/21 00:24:22 alc Exp $
 */
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
Tmk_lock_acquire(0);
{
DEBUG('l',"Acquiring lock memory1\n");
...
UINT	size;
	...
	size	     = ROUND_UP(size);
	gm->freelist = (NODE huge *)Tmk_malloc(size);;
...
Tmk_lock_acquire(0);
{	
DEBUG('l',"Acquiring lock memory2\n");
	prev = NULL;
	curr = gm->freelist;
	size = ROUND_UP(size);
...
Tmk_lock_acquire(0);
{
	DEBUG('l',"Acquiring lock memory3\n");
...
{
	DEBUG('l',"Acquiring lock memory4\n");
...
Tmk_lock_acquire(0);
{
	DEBUG('l',"Acquiring lock memory5\n");
...
{
	DEBUG('l',"Acquiring lock memory6\n");
...
INT	count;
	...
	switch (ObjectType)
		...
		case OT_EMPTYCELLS:
			...
			n = w*sizeof(UINT);
			p = GlobalMallocSec(n, "EMPTYCELLS");
			x = p;
