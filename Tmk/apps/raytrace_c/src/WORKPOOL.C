...
INT	pid;
	...
	for (i = 0; i < ye; i += ybe)
		...
		for (j = 0; j < xe; j += xbe)
			...
			xb_addr += xbe;
			DEBUG('w',"This job %p, previous job %p\n",
			          gm->workpool[pid][0],gm->workpool[pid][0]->next);
		      }
...
Tmk_lock_acquire(10+pid);

{
	wpentry = gm->workpool[pid][0];

	DEBUG('l',"Acquiring lock workpool\n");
...
INT	pid;
	...
	INT	xsize, ysize;
        ...
        DEBUG('c',"In InitWorkPool\n");

        for (i=0; i<gm->nprocs; i++) {
	  gm->wpstat[i][0]   = WPS_VALID;
	  gm->workpool[i][0] = NULL;
	}
	...
	ysize  = Display.yres/blocky;
        ...
        DEBUG('d',"Display resolution x %d y %d\n",Display.xres,Display.yres);
...
/***************************************
			if (i == pid) {
			  ...
			  DEBUG('j',"Inserting job x %d y %d\n",x,y);
			}
***************************************/
			PutJob(x, y, xe, ye, bundlex, bundley, i);
