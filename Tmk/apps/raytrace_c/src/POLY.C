...
FILE	*pf;
	...
	pp    = GlobalMallocSec(sizeof(POLY)*po->numelements,  "poly.c");
	vlist = GlobalMallocSec(sizeof(VEC3)*(totalverts + 1), "poly.c");
	vptr  = vlist;
	...
	for (i = 0; i < po->numelements; i++)
		...
		pp->vptr   = vlist;
		pp->vindex = GlobalMallocSec(sizeof(INT)*pp->nverts, "poly.c");
		vindex	   = pp->vindex;
