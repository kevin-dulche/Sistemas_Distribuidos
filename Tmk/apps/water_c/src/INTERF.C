...
#define	LOCAL_COMP
#ifdef	LOCAL_COMP
molecule_type	localVAR[MAXMOLS];
#endif	/*LOCAL_COMP*/
...
*/
...
#ifdef	LOCAL_COMP
    memset(localVAR, 0, NMOL*sizeof(molecule_type));
#endif	/*LOCAL_COMP*/
...
#ifdef	LOCAL_COMP
    {
	int		i, start = StartMol[Tmk_proc_id];
	...
	for (i = 0; i < NMOL; i++)
	{
	    ...
	    {
		Tmk_lock_acquire(gl->MolLock[mol]);
		...
		Tmk_lock_release(gl->MolLock[mol]);
	    }
	}
    }
#endif	/*LOCAL_COMP*/
    ...
    Tmk_lock_acquire(gl->InterfVirLock);
    *VIR = *VIR + LVIR;
    Tmk_lock_release(gl->InterfVirLock);

            /* wait till all forces are updated */
if (Tmk_nprocs > 1)
    Tmk_barrier(gl->InterfBar);
...
double XL[], YL[], ZL[], FF[];
{
    ...
    localVAR[comp].F[DEST][XDIR][O] +=
    	-G110[XDIR]-GG[13][XDIR]-GG[14][XDIR]-C1*G45[XDIR];
    localVAR[comp].F[DEST][XDIR][H1] +=
    ...
    Tmk_lock_acquire(gl->MolLock[comp]);
    ...
    VAR[comp].F[DEST][ZDIR][H2] +=
    	-GG[7][ZDIR]-GG[9][ZDIR]-GG[12][ZDIR]-TT2[ZDIR]-GG[3][ZDIR];
    Tmk_lock_release(gl->MolLock[comp]);
#endif
